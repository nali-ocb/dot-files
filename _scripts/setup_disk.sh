...
#
# NOTE: if on `sudo systemctl status wpa_supplicant`, you get "rfkill: WLAN soft blocked", try:
# `rfkill list`
# `rfkill unblock wlan`
#
# NOTE: You can use scan / scan_results to see the list of ssid
#
# `wpa_cli`
# > interface wlp2s0
# OK
...
# The only difference is that I'm using tempfs and
# I decided to keep /var stored, because of the size and
# because it keeps a lot of cache that I want between reboots
